...
public class AggregateGeomFunction {
    ...
    public static AccumulatorFactory unionAccumulatorFactory = new AccumulatorFactory() {
        @Override
        public Accumulator createAccumulator(AggCustom agg, boolean distinct) { return new UnionAccumulator(agg); }
    };

    public static AccumulatorFactory intersectionAccumulatorFactory = new AccumulatorFactory() {
        @Override
        public Accumulator createAccumulator(AggCustom agg, boolean distinct) { return new IntersectionAccumulator(agg); }
    };

    public static AccumulatorFactory envelopeAccumulatorFactory = new AccumulatorFactory() {
        @Override
        public Accumulator createAccumulator(AggCustom agg, boolean distinct) { return new EnvelopeAccumulator(agg); }
    };

    private static void collectGeometries(Binding binding, FunctionEnv functionEnv, AggCustom agg, WktParser parser, ArrayList<Geometry> geometries) {
        ExprList exprList = agg.getExprList();
        for(Expr expr: exprList) {
            try {
                NodeValue nv = expr.eval(binding, functionEnv);
                if (nv.isLiteral()) {
                    Geometry poly = parser.parseGeometry(nv.asString());
                    geometries.add(poly);
                }
            } catch (ExprEvalException | ParseException ex) {
                ex.printStackTrace();
            }
        }
    }
    ...
    static class UnionAccumulator implements Accumulator {
        ...
        public void accumulate(Binding binding, FunctionEnv functionEnv) {
            collectGeometries(binding, functionEnv, agg, parser, geometries);
        }
